...
Modeling a Distance Runner's Race with Interpolation}
...
nodes and the $(n+1)$ derivative of $f(t)$. When the nodes are equally
spaced and $n$ is large, the product $\prod_{i=0}^{n}(t-t_i)$ becomes
large near the ends of the interval, so any oscillations in the high--degree
interpolating polynomial translate into large interpolation errors near
the endpoints (Runge's phenomenon).
...
\begin{enumerate}
  \item $S(t_i) = s_i$ for $i = 0,\dots,n$, so $S(t)$ is continuous at
        each knot and interpolates all of the data,
...
Figure~\ref{fig:splits} displays the official split times for this runner.
...
MATLAB driver script used to generate our figures. In total there are
$n+1$ checkpoints (including the start and the finish), corresponding to
standard marathon timing mats; they are not equally spaced in either
time or distance.
...
For the runner we model, all three interpolants agree that the pace is
nearly constant through the middle portion of the marathon with a modest
increase in speed near the finish, a pattern consistent with a
well-controlled race followed by a final push. Overall, while all
methods reconstruct the position data accurately, the differentiation
process magnifies modeling assumptions. The results demonstrate that
careful selection of spline boundary conditions is essential when
derivative quantities such as velocity and acceleration are of primary
interest. Among the interpolants tested, the clamped cubic spline
provides the most balanced and physically realistic representation of
the runner's pacing profile.
