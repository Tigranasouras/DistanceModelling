...
For each $S(t)$, we compute the velocity and acceleration by
...
The first plot compares the Newton interpolating polynomial and the
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.85\linewidth]{PosvsTime}
  \caption{Cumulative distance versus time for each interpolation method.
  Both interpolants pass through the split data, but the spline remains
  slightly smoother near the end of the race.}
  \label{fig:position}
\end{figure}

\subsection{Velocity and Acceleration Profiles}
Differentiating each gives the velocity curves. Both show an initial acceleration
phase, a roughly steady middle section, and a decrease toward the end of
the race. However, the polynomial-based velocity reaches larger peaks
and drops off more steeply near the finish, while the spline-based
velocity changes more gradually. -As we would expect to occur.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.85\linewidth]{VelvsTime}
  \caption{Velocity versus time obtained from the polynomial and spline
  interpolants. The polynomial exhibits larger peaks and a sharper drop
  near the finish compared to the spline.}
  \label{fig:velocity}
\end{figure}

The second derivative plots show the same pattern more clearly. The
spline acceleration is small and slowly varying for most of the race,
consistent with small adjustments in pace. The polynomial acceleration
shows larger positive and negative swings, especially near the final
split, which do not correspond to any obvious features in the original
data.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.85\linewidth]{AccelvsTime}
  \caption{Acceleration versus time from each interpolant. The natural
  cubic spline gives moderate, slowly varying accelerations, while the
  polynomial shows oscillations that are less physically realistic.}
  \label{fig:acceleration}
\end{figure}

\section{Discussion}
This supports the conclusion that spline interpolation is better suited
than a single polynomial for modeling a runner’s race from discrete
splits. The polynomial is highly sensitive to the placement of
nodes and develops oscillations near the boundary, which leads to
unrealistic behavior in the velocity and acceleration plots. In
contrast, the natural cubic spline uses smoothness conditions, producing more physically reasonable pacing profiles.
...
produces the most realistic pacing curves. High-degree polynomials can
oscillate and generate unrealistic derivatives.
